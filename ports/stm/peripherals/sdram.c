...
#if CIRCUITPY_DEBUG_VERBOSE // print debugging info
...
static void sdram_init_seq(SDRAM_HandleTypeDef
    *hsdram, FMC_SDRAM_CommandTypeDef *command) {
    ...
    tmpmrd = (uint32_t)FMC_INIT(SDRAM_MODEREG_BURST_LENGTH, CIRCUITPY_HW_SDRAM_BURST_LENGTH) |
...
bool __attribute__((optimize("Os"))) sdram_test(bool exhaustive) {
    ...
    DEBUG_printf("info: %s", "sdram test started");
    ...
    for (uint32_t i = 0; i < CIRCUITPY_HW_SDRAM_MEM_BUS_WIDTH; i++) {
        *((volatile uint32_t *)mem_base) = (1u << i);
        ...
        if (*((volatile uint32_t *)mem_base) != (1u << i)) {
            ...
            DEBUG_printf("error: %s", error_buffer);
            return false;
        }
    }
    ...
    DEBUG_printf("info: %s", "sdram test successfully!");
